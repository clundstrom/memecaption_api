...
class MemeGenerator:
        ...
        for i in range(len(boxes)):
            ...
            if i == 0:
                for line in wrap:
                    ...
                    draw.text((anchor[0], anchor[1]), line, "black", adj_font, stroke_width=2, stroke_fill="white")
                    anchor[1] = anchor[1] + height
            else:
                for line in wrap:
                    width, height = font.getsize(line)
                    anchor = self.setTextAnchor(boxes[i].x, boxes[i].y, 'lower', img, draw.textsize(line, adj_font)[0])
                    draw.text((anchor[0], anchor[1]), line, "black", adj_font, stroke_width=2,
                              stroke_fill="white")
                    anchor[1] = anchor[1] + height
    ...
    def setTextAnchor(self, x_offset, y_offset, boxposition, img, textsize):
        ...
        if boxposition == 'upper':
            x = ((img.size[0] / 2) + x_offset) - ((textsize / 2)+x_offset)
